...
public final class JsonWriterTest {
  ...
  @Test
  public void testValueWithFiniteFloat() throws IOException {
      StringWriter stringWriter = new StringWriter();
      JsonWriter jsonWriter = new JsonWriter(stringWriter);
      jsonWriter.setStrictness(Strictness.STRICT);
      jsonWriter.beginArray();
      jsonWriter.value(3.14f);
      jsonWriter.endArray();
      jsonWriter.close();
  ...
  @Test
  public void testValueWithNonFiniteFloatInStrictMode() throws IOException {
      StringWriter stringWriter = new StringWriter();
      JsonWriter jsonWriter = new JsonWriter(stringWriter);
      jsonWriter.setStrictness(Strictness.STRICT);
      ...
      assertThat(exception).hasMessageThat().isEqualTo("Numeric values must be finite, but was NaN");

      exception = assertThrows(
              IllegalArgumentException.class,
              () -> jsonWriter.value(Float.POSITIVE_INFINITY)
      );
      assertThat(exception).hasMessageThat().isEqualTo("Numeric values must be finite, but was Infinity");
