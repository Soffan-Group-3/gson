 ...
 public class JsonReader implements Closeable {
   ...
   int doPeek() throws IOException {
     ...
     if (peekStack == JsonScope.EMPTY_ARRAY) {
       ...
       int result = handleNonEmptyArraySeparator();
       if (result != 0) return result;
     } else if (peekStack == JsonScope.EMPTY_OBJECT || peekStack == JsonScope.NONEMPTY_OBJECT) {
       stack[stackSize - 1] = JsonScope.DANGLING_NAME;
       if (peekStack == JsonScope.NONEMPTY_OBJECT) {
         int result = handleObjectSeparator();
         if (result != 0) return result;
       }
       return readNameToken(peekStack);
     ...
     int valueToken = peekValueToken(peekStack);
     if (valueToken != PEEKED_NONE) return valueToken;
   ...
   /** Handles the separator logic for a non-empty array. */
   private int handleNonEmptyArraySeparator() throws IOException {
     int c = nextNonWhitespace(true);
     switch (c) {
       case ']':
   ...
   /** Handles the separator logic for a non-empty object. */
   private int handleObjectSeparator() throws IOException {
     int c = nextNonWhitespace(true);
     switch (c) {
       case '}':
   ...
   /** Reads the next name token for an object. */
   private int readNameToken(int peekStack) throws IOException {
     int c = nextNonWhitespace(true);
     switch (c) {
       case '"':
   ...
   /** Processes literal and structural tokens following scope handling. */
   private int peekValueToken(int peekStack) throws IOException {
     int c = nextNonWhitespace(true);
     switch (c) {
       case ']':
         if (peekStack == JsonScope.EMPTY_ARRAY) return peeked = PEEKED_END_ARRAY;
       // fall-through for cases like ",]"
       case ';':
       case ',':
